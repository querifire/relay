...
async fn connect_through_http(
    ...
    loop {
        tokio::time::timeout(HEADER_READ_TIMEOUT, stream.read_exact(&mut buf))
            .await
            .map_err(|_| anyhow!("Upstream HTTP header read timeout"))??;
        response.push(buf[0]);
...
async fn connect_through_socks4(
    ...
    let socks_stream =
        tokio_socks::tcp::Socks4Stream::connect_with_socket(stream, target_addr).await?;
...
async fn connect_through_socks5(
    ...
    let target_addr = format!("{}:{}", target_host, target_port);
    let socks_stream =
        tokio_socks::tcp::Socks5Stream::connect_with_socket(stream, target_addr).await?;
